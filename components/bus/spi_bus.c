...
esp_err_t spi_bus_delete(spi_bus_handle_t *p_bus_handle)
{
    ...
    SPI_BUS_CHECK(ESP_OK == ret, "spi bus del failed", ESP_FAIL);
    ESP_LOGI(TAG, "SPI%d bus del", spi_bus->host_id + 1);
...
esp_err_t spi_bus_device_delete(spi_bus_device_handle_t *p_dev_handle)
{
    ...
    SPI_BUS_CHECK(ESP_OK == ret, "spi bus del device failed", ret);
