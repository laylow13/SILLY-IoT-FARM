...
static void sensor_default_task(void *arg)
{
    ...
    while (arg) { /*arg == NULL is invalid, task will be deleted*/
        ...
        if ((uxBits & BIT23_KILL_WAITING_TASK) != 0) { /*task del event*/
            break;
        }
    ...
    ESP_LOGI(TAG, "task: del sensor_default_task !");
    vTaskDelete(NULL);
}
...
esp_err_t iot_sensor_delete(sensor_handle_t *p_sensor_handle)
{
    ...
    switch (sensor->mode) {
        case MODE_POLLING:
            SENSOR_CHECK(pdTRUE == xTimerDelete(sensor->timer_handle, portMAX_DELAY), "sensor del failed", ESP_FAIL);
            sensor->timer_handle = NULL;
            break;

        case MODE_INTERRUPT:
            if (sensor->isr_state == ISR_STATE_ACTIVE){
                SENSOR_CHECK(ESP_OK == sensor_intr_isr_remove(sensor->intr_pin), "sensor del failed", ESP_FAIL);
                sensor->isr_state = ISR_STATE_INACTIVE;
            }
            break;

        default:
            SENSOR_CHECK( false, "sensor del failed", ESP_ERR_NOT_SUPPORTED);
            break;
    }
    ...
    /*set sensor to sleep mode then del the driver*/
    ret = sensor->impl->control(sensor->driver_handle, COMMAND_SET_POWER,  (void *)POWER_MODE_SLEEP);
    ...
    ret = sensor->impl->del (&sensor->driver_handle);
    SENSOR_CHECK(ret == ESP_OK && sensor->driver_handle == NULL, "sensor driver del failed", ret);

    /*free the resource then set handle to NULL*/
    free(sensor);
    *p_sensor_handle = NULL;

    /*if no event bits to wait, del the default sensor task*/
    if (s_sensor_wait_bits == BIT23_KILL_WAITING_TASK) {
        xEventGroupSetBits(s_event_group, BIT23_KILL_WAITING_TASK); /*set bit to del the task*/
